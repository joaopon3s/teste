...
/* ── PRICING TOGGLE ── */
// Posição [0] = Parcelado (10x), Posição [1] = À vista (com desconto)
const prices = {
  starter: [29, 29],
  pro: [99, 890],
  ent: [150, 1350]
...
function updatePricing() {
  ...
  // Elementos do texto de período e notas informativas
  const periodPro = document.getElementById('period-pro');
  const periodEnt = document.getElementById('period-ent');
  const notePro = document.getElementById('annual-note-pro');
  const noteEnt = document.getElementById('annual-note-ent');

  if (isAnnual) {
    // Texto para pagamento À Vista
    if (periodPro) periodPro.innerHTML = '&nbsp; no PIX';
    if (periodEnt) periodEnt.innerHTML = '&nbsp; no PIX';
    if (notePro) notePro.textContent = 'Economize R$ 100';
    if (noteEnt) noteEnt.textContent = 'Economize R$ 150,00 com 10% de desconto';
  } else {
    // Texto para pagamento Parcelado
    if (periodPro) periodPro.innerHTML = '&nbsp;em 10x s/ juros';
    if (periodEnt) periodEnt.innerHTML = '&nbsp;em 10x s/ juros';
    if (notePro) notePro.textContent = 'Valor total: R$ 990';
    if (noteEnt) noteEnt.textContent = 'Total parcelado: 10x de R$ 150,00';
  }
